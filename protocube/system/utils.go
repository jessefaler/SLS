...
func ScanReader(r io.Reader, callback func(line []byte)) error {
	...
	for {
		...
		line = bytes.ReplaceAll(line, cr, crr)
			...
			// long lines to grow the buffer size excessively and potentially DOS the daemon
			// instance. If the line is not too long, just store the whole value into theF
			// buffer. This is kind of a re-implementation of the bufio.Scanner.Scan() logic
			// without triggering an error when you exceed this buffer size.S
			if ns > maxBufferSize {
