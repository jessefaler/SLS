...
/**
 * {@link com.protoxon.S4J.SLSAction SLSAction} specification used to retrieve entities for paginated endpoints.
 ...
 *     <br>These functions act like standard {@link List Lists}. Unlike the latter, calling these functions
 ...
 *     <br>The remaining functions are similar to {@link Queue#poll polling} from a {@link Queue Queue} while iterating.
 ...
 */
public interface PaginationAction<T> extends SLSAction<List<T>>, Iterable<T> {
	 ...
	 * <br>Every {@link com.protoxon.S4J.SLSAction SLSAction} success
	 ...
	 * retrieved by the iterator or a call to a {@link com.protoxon.S4J.SLSAction SLSAction}
	...
	default CompletableFuture<?> forEachAsync(Procedure<? super T> action) {
		return forEachAsync(action, SLSAction.getDefaultFailure());
	}
	...
	default CompletableFuture<?> forEachRemainingAsync(Procedure<? super T> action) {
		return forEachRemainingAsync(action, SLSAction.getDefaultFailure());
	}
	 ...
	 *
	 * @param  action
	 *         The {@link com.protoxon.S4J.utils.Procedure Procedure}
	 ...
	 */
	default SLSAction<List<T>> all() {
		return SLSActionImpl.onExecute(getS4J(), () -> stream().collect(StreamUtils.toUnmodifiableList()));
	}
