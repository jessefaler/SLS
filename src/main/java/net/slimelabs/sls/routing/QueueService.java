...
public class QueueService {
    ...
    public void queuePlayerToJoinServer(String serverName, ServerInstance serverInstance, CommandSource source) {
        ...
        ProtoMessage.chat().add(MessagePreset.SLS).add("In queue for " + serverName.replace("_", " "), NamedTextColor.DARK_AQUA).sendMessage(source);
        task = SLS.PROXY.getScheduler().buildTask(SLS.PLUGIN, () -> {
            ...
            if (serverInstance.state == UtilizationState.RUNNING) {
                enableActionBarPackets(player.getUniqueId());
                ProtoMessage.actionBar().add("Joining " + serverName.replace("_", " "), NamedTextColor.GREEN).sendMessage(source);
            ...
            if (serverInstance.shutdown) {
                ProtoMessage.chat().add(MessagePreset.SLS).add("Failed to join " + serverName.replace("_", " "), NamedTextColor.RED).sendMessage(source);
                SLS.PLAYER_CONNECTOR.queue.remove(uuid);
                SLS.PLAYER_CONNECTOR.activeServices.remove(player.getUniqueId());
                enableActionBarPackets(player.getUniqueId());
                task.cancel();
            ...
            if (max > 1714) { // Request times out in ~2 minutes
                ProtoMessage.chat().add(MessagePreset.SLS).add("Failed to join " + serverName + ". Queue timed out.", NamedTextColor.RED).sendMessage(source);
                enableActionBarPackets(player.getUniqueId());
                task.cancel();
                return;
            }
